...
func _process(delta):
	...
	if playerData && Input.is_action_just_pressed("ui_E") && !taskOcorring && !Global.dataTaskComplete && Global.energy > 0 || playerData && Global.ePressed && !taskOcorring && !Global.dataTaskComplete && Global.energy > 0:
