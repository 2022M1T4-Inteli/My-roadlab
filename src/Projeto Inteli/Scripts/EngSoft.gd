...
func _process(delta):
	...
	if playerAutomationTest && Input.is_action_just_pressed("ui_E") && !taskOcorring && !Global.automationTestTaskComplete && Global.energy > 0 || playerAutomationTest && Global.ePressed && !taskOcorring && !Global.automationTestTaskComplete && Global.energy > 0:
